...
############### M3 routing ###########################
        for i in range(3):
            if x == 0 and y_cells > 1 and i == 0:
                ...
                uc.v2Segment( 'v2', 'NA', (i*m1Pitch + m1Offset - v1Width/2), (i*m1Pitch + m1Offset + v1Width -   v1Width/2), (y*(m2_tracks //y_cells + K_space) + finDummy//2 + 1))
            ...
            if x == 0 and y_cells > 1 and i == 1:
                ...
                uc.v2Segment( 'v2', 'NA', (i*m1Pitch + m1Offset - v1Width/2), (i*m1Pitch + m1Offset + v1Width -   v1Width/2), (y*(m2_tracks //y_cells + K_space) + finDummy//2 + 0))

            if x == 0 and y_cells > 1 and i == 2:
                if y == 0:
                    uc.m3Segment( 'm3', 'D', (i+(x*gate)), ((m2Pitch*(finDummy//2-1)) - v_enclosure), ((y_cells-1)*m2Pitch*(K_space + fin//2) - v_enclosure + m1Length + (m2Pitch*(finDummy//2))))
                uc.v2Segment( 'v2', 'NA', (i*m1Pitch + m1Offset - v1Width/2), (i*m1Pitch + m1Offset + v1Width -   v1Width/2), (y*(m2_tracks //y_cells + K_space) + finDummy//2 + 2))
...
############### M2 routing ###########################
        for i in range((m2_tracks+1)):
            ...
            if x_cells > 1 and x == 0 and i == (y*(m2_tracks //y_cells + K_space) + finDummy//2 + 2):
                uc.m2Segment( 'm2', 'D', (m1Offset - m1Width/2 - v_enclosure), (m1Offset + m1Width/2 + v_enclosure + (x_cells*gate-2)*m1Pitch), i)
...
################# M1 routing ######################
        if (x_cells - 1 - x) == 0:
            for i in GA:
                ...
                uc.m1Segment( 'm1', 'G', i, (0 - v_enclosure + (m2Pitch*(finDummy//2-1)) + y*m2Pitch*(K_space + fin//2) ), (y*m2Pitch*(K_space + fin//2)+ (m2Pitch*(finDummy//2-1)) - v_enclosure + m1PCLength))                           
            for i in SA:
            ...
            for i in DA:
                ...
                uc.m1Segment( 'D', 'D', i, ((m2Pitch*(finDummy//2-1)) - v_enclosure + y*m2Pitch*(K_space + fin//2) ), (y*m2Pitch*(K_space + fin//2) - v_enclosure + m1Length + (m2Pitch*(finDummy//2-1))))
