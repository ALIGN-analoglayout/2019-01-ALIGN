...
############### M3 routing ###########################
        for i in range(3):
            ...
            if x == 0 and y_cells > 1 and i == 1:
                if y == 0:
                    uc.m3Segment( 'm3', 'GA', (i+(x*gate)), ((m2Pitch*(finDummy//2-1)) - v_enclosure), ((y_cells-1)*m2Pitch*(K_space + fin//2) - v_enclosure + m1Length + (m2Pitch*(finDummy//2))))
            ...
            if x == 1 and y_cells > 1 and i == 1:
                if y == 0:
                    uc.m3Segment( 'm3', 'GB', (i+(x*gate)), ((m2Pitch*(finDummy//2-1)) - v_enclosure), ((y_cells-1)*m2Pitch*(K_space + fin//2) - v_enclosure + m1Length + (m2Pitch*(finDummy//2))))
...
############### M2 routing ###########################
        for i in range((m2_tracks+1)):
            ...
            if x_cells > 1 and x == 0 and i == (y*(m2_tracks //y_cells + K_space) + finDummy//2 + 0):
                uc.m2Segment( 'm2', 'GA', (m1Offset - m1Width/2 - v_enclosure), (m1Offset + m1Width/2 + v_enclosure + (x_cells*gate-2)*m1Pitch), i)

            if x_cells > 1 and x == 0 and i == (y*(m2_tracks //y_cells + K_space) + finDummy//2 + 3):
                uc.m2Segment( 'm2', 'GB', (m1Offset - m1Width/2 - v_enclosure), (m1Offset + m1Width/2 + v_enclosure + (x_cells*gate-2)*m1Pitch), i)
        ...
        if (x_cells - 1 - x) == 0:
            if (y % 2) == 0:
                ...
                for i in GA:
                    ...
                    uc.m1Segment( 'm1', 'GA', i, (0 - v_enclosure + (m2Pitch*(finDummy//2-1)) + y*m2Pitch*(K_space + fin//2) ), (y*m2Pitch*(K_space + fin//2)+ (m2Pitch*(finDummy//2-1)) - v_enclosure + m1PCLength))
                for i in GB:
                    ...
                    uc.m1Segment( 'm1', 'GB', i, (0 - v_enclosure + (m2Pitch*(finDummy//2-1)) + y*m2Pitch*(K_space + fin//2) ), (y*m2Pitch*(K_space + fin//2)+ (m2Pitch*(finDummy//2-1)) - v_enclosure + m1PCLength))      
            else:
                for i in DA:
                    uc.v1Segment( 'v1', 'NA', (i*m1Pitch + m1Offset - v1Width/2), (i*m1Pitch + m1Offset + v1Width -   v1Width/2), (y*(m2_tracks //y_cells + K_space) + finDummy//2 + 4))
                    uc.m1Segment( 'D', 'DA', i, ((m2Pitch*(finDummy//2-1)) - v_enclosure + y*m2Pitch*(K_space + fin//2) ), (y*m2Pitch*(K_space + fin//2) - v_enclosure + m1Length + (m2Pitch*(finDummy//2-1))))
